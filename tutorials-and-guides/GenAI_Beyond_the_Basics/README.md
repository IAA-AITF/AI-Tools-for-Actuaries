...
# Tutorial: GenAI Beyond the Basics
...
Throughout the notebook, you will work with example datasets (mortality tables and synthetic car insurance policies) and see how these GenAI building blocks can be combined into richer workflows such as real-time claim assessment or multi-agent systems.
...
```bash
git clone https://github.com/IAA-AITF/AI-Tools-for-Actuaries.git
cd AI-Tools-for-Actuaries/tutorials-and-guides/GenAI_Beyond_the_Basics
...
> _To run the examples, you will need a valid OpenAI API key (e.g. set via an environment variable such as `OPENAI_API_KEY`). The notebook can be adjusted to work with other LLM providers._
...
- **APIs as a Gateway to GenAI** — Integrate LLM calls into Python workflows so that GenAI becomes a standard component in actuarial tools rather than a standalone interface.
- **Structured Outputs and Auditability** — Use JSON schemas and structured outputs to ensure responses can be validated, stored, and reused in downstream actuarial models and dashboards.
- **Function Calling for "Thinking + Doing"** — Let the model decide *what* information is needed, while deterministic functions (e.g. database lookups for mortality tables) handle *how* to get it.
- **Fine-Tuning for Actuarial Specialisation** — Fine-tuned, domain-specific models can improve accuracy in tasks such as car damage classification, underwriting text review, or claims triage.
- **RAG for Knowledge-Rich Use Cases** — Combine LLMs with document stores to extract KPIs from long annual reports or regulatory texts without hitting context limits.
- **Blueprints for Future Systems** — These building blocks can be combined into end-to-end solutions such as real-time hail-claim assessment or multi-agent setups.
...
Simon Hatzesberger ([simon.hatzesberger@gmail.com](mailto:simon.hatzesberger@gmail.com))
